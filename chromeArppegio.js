    ...
    Main(function() {
        ...
        function toNotationHelper(t, e, i, s) {
            ...
            for (n = 0; n < s.length; n++)
                if (o = this.notationToSeconds(s[n], e, i), r = l / o, a = 1e-6, a > 1 - r % 1 && (r += a), r = Math.floor(r), r > 0) {
        ...
        return Tone.Type = {
            ...
            if (this.isString(time)) {
                if (plusTime = 0, this.isNowRelative(time) && (time = time.replace("+", ""), plusTime = now), betweenParens = time.match(/\(([^)(]+)\)/g))
                    for (j = 0; j < betweenParens.length; j++) symbol = betweenParens[j].replace(/[\(\)]/g, ""), symbolVal = this.toSeconds(symbol), time = time.replace(betweenParens[j], symbolVal);
                if (-1 !== time.indexOf("@")) {
        ...
        return t.Expr = function() {
            ...
            function i(e) {
                ...
                for (i in t.Expr._Expressions) {
                    ...
                    for (n in s)
                        if (o = s[n], r = o.regexp, a = e.match(r), null !== a) return {
                            type: i,
                            value: a[0],
                            method: o.method
                        }
            ...
            for (s = 0; s < r.length; s++)
                if (e = r[s], this._events.hasOwnProperty(e))
                    if (t.prototype.isUndef(i)) this._events[e] = [];
                        ...
                        for (n = this._events[e], o = 0; o < n.length; o++) n[o] === i && n.splice(o, 1);
            return this
        }, t.Emitter.prototype.trigger = function(t) {
            var e, i, s, n;
            if (this._events && (e = Array.prototype.slice.call(arguments, 1), this._events.hasOwnProperty(t)))
                for (i = this._events[t], s = 0, n = i.length; n > s; s++) i[s].apply(this, e);
            return this
        ...
        t.IntervalTimeline = function() {
            ...
            var e, i, s;
            if (null !== this._root)
                for (e = [], null !== this._root && this._root.traverse(function(t) {
                        e.push(t)
            ...
            if (t = this.toSeconds(t), null !== this._root)
                for (i = [], this._root.search(t, i), s = i.length - 1; s >= 0; s--) n = i[s].event, n && e(n);
            return this
        }, t.IntervalTimeline.prototype.forEachAfter = function(t, e) {
            var i, s, n;
            if (t = this.toSeconds(t), null !== this._root)
                for (i = [], this._root.searchAfter(t, i), s = i.length - 1; s >= 0; s--) n = i[s].event, n && e(n);
            return this
        ...
        return t.CtrlMarkov = function(t, e) {
            ...
            if (r)
                for (s = 0; s < n.length; s++) n[s] = n[s] / o;
            return n
        ...
        function e(t, e, s) {
            ...
            if (i.hasOwnProperty(t))
                for (n = i[t], o = 0, r = n.length; r > o; o++) a = n[o], Array.isArray(s) ? a.apply(window, [e].concat(s)) : a(e, s)
        }
        ...
        return t.Part = function() {
            ...
            if (this._loop = s.loop, this._loopStart = this.toTicks(s.loopStart), this._loopEnd = this.toTicks(s.loopEnd), this._playbackRate = s.playbackRate, this._probability = s.probability, this._humanize = s.humanize, this._startOffset = 0, this._state = new t.TimelineState(t.State.Stopped), this._events = [], this.callback = s.callback, this.mute = s.mute, e = this.defaultArg(s.events, []), !this.isUndef(s.events))
                for (i = 0; i < e.length; i++) Array.isArray(e[i]) ? this.add(e[i][0], e[i][1]) : this.add(e[i])
            ...
            var i, s, n;
            for (t = this.toTicks(t), i = this.ticksToSeconds(1), s = 0; s < this._events.length; s++)
                if (n = this._events[s], Math.abs(t - n.startOffset) < i) return this.isUndef(e) || (n.value = e), n;
        ...
        return t.Sequence = function() {
            var e, i = this.optionsObject(arguments, ["callback", "events", "subdivision"], t.Sequence.defaults),
                s = i.events;
            if (delete i.events, t.Part.call(this, i), this._subdivision = this.toTicks(i.subdivision), this.isUndef(i.loopEnd) && !this.isUndef(s) && (this._loopEnd = s.length * this._subdivision), this._loop = !0, !this.isUndef(s))
        ...
        return t._initAudioContext(function(t) {
            ...
            e = function() {
                ...
                for (e = 0; e < d.numberOfChannels; e++)
                    for (i = d.getChannelData(e), s = r = a = l = h = u = c = 0, p = 0; o > p; p++) f = 2 * Math.random() - 1, s = .99886 * s + .0555179 * f, r = .99332 * r + .0750759 * f, a = .969 * a + .153852 * f, l = .8665 * l + .3104856 * f, h = .55 * h + .5329522 * f, u = -.7616 * u - .016898 * f, i[p] = s + r + a + l + h + u + c + .5362 * f, i[p] *= .11, c = .115926 * f;
                return d
            }(), i = function() {
                var e, i, s, r, a, l = t.createBuffer(2, o, n);
                for (e = 0; e < l.numberOfChannels; e++)
                    for (i = l.getChannelData(e), s = 0, r = 0; o > r; r++) a = 2 * Math.random() - 1, i[r] = (s + .02 * a) / 1.02, s = i[r], i[r] *= 3.5;
                return l
            }(), s = function() {
